export default function links(puzzleMap) {
    const mapOut = new Map();
    for (let y = 0; y < puzzleMap.length; y++) {
        for (let x = 0; x < puzzleMap[y].length; x++) {
            const check_dot = (x, y) => puzzleMap[y][x] != ".";
            const check_left = () => (x != 0 ? check_dot(x - 1, y) : false);
            const check_right = () => x != puzzleMap[y].length - 1 ? check_dot(x + 1, y) : false;
            const check_up = () => (y != 0 ? check_dot(x, y - 1) : false);
            const check_down = () => y != puzzleMap.length - 1 ? check_dot(x, y + 1) : false;
            if (mapOut.get(x) == undefined) {
                mapOut.set(x, new Map());
            }
            if (check_dot(x, y)) {
                const store = (bool) => { var _a; return (_a = mapOut.get(x)) === null || _a === void 0 ? void 0 : _a.set(y, bool); };
